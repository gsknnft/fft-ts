...
class FourierTransform {
  ...
  constructor(bufferSize: number, sampleRate: number, data?: Float64Array) {
    this.data = data || new Float64Array(bufferSize);
  ...
  transform() {
    ...
    fft(1, 1, size, buffer, x_ptr, y_ptr, scratch_ptr);

    this.real.set(buffer.subarray(x_ptr, x_ptr + size));
    this.imag.set(buffer.subarray(y_ptr, y_ptr + size));
  }
...
function fft(dir: number, nrows: number, ncols: number, buffer: Float64Array<ArrayBufferLike>, x_ptr: number, y_ptr: number, scratch_ptr: number) {
  dir |= 0
  nrows |= 0
  ncols |= 0
  x_ptr |= 0
...
function scratchMemory(n: number) {
  if(bits.isPow2(n)) {
    return 0
  }
...
//const { spectrum, real, imag } = FourierTransform.transform(inputArray, sampleRate);
/* 
const transform = new FourierTransform(size, sampleRate);
transform.real.set(inputArray);
transform.transform(); // Internally picks Radix-2 or Bluestein
const spectrum = transform.calculateSpectrum();
