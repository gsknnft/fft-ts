...
### Fixed (correctness â€” the core FFT was producing wrong/throwing results)

- **`FFT` constructor was broken.** It read `_csize` before assigning `size`
  (â†’ zero-length buffers â†’ `Invalid typed array length: -2147483648`) and built
...
- **`fft` export was the class, not a function** â€” `fft(data)` threw
...
- **`FFTProcessor` passed a buffer where a numeric size was required** â€” fixed to the
  derived size.
- **`fftFunc` / `magnitudeFromComplex` / `forwardMagnitudes` / `deriveCoherence` all
  crashed** â€” they indexed a struct-of-arrays `ComplexArray` (`.real[i]` / `.imag[i]`)
...
- **`fft(input)`** â€” high-level forward FFT for arbitrary-length real input. Pads to the
  next power of two; returns the full interleaved complex spectrum.
- **`fftMagnitude(input)`** â€” magnitude spectrum of real input, one magnitude per bin.
